package com.estoque.api.handler;

import com.estoque.api.exception.generico.ErroDeIntegridadeException;
import com.estoque.api.exception.generico.OperacaoNaoPermitidaException;
...
public class GlobalExceptionHandler {
  ...
  public ResponseEntity<String> handleHttpMessageNotReadable(HttpMessageNotReadableException ex) {
    if (ex.getCause() instanceof InvalidFormatException invalidFormat
      ...
      String enumValues =
          Arrays.stream(invalidFormat.getTargetType().getEnumConstants())
              .map(Object::toString)
              .collect(Collectors.joining(", "));

      String msg =
          String.format(
              "Valor inválido para o campo '%s'. Valores válidos: [%s]", field, enumValues);
      return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(msg);
    }

    return ResponseEntity.status(HttpStatus.BAD_REQUEST)
        .body("Erro de leitura do corpo da requisição.");
  }
  ...
  public ResponseEntity<Map<String, String>> handleQuantidadeInsuficienteException(
